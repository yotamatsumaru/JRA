...
<div class="space-y-4">
    ...
    <form method="GET" x-data="{ open: window.innerWidth >= 640 }" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 p-4">
        ...
        <div x-show="open" x-transition.opacity class="flex flex-wrap gap-3 text-sm items-end">
            ...
            <a href="{{ route('jockeys.index') }}" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 px-3 py-1.5 text-xs">クリア</a>
        </div>
    </form>
        ...
        <table class="w-full text-sm min-w-[640px]">
            <thead class="bg-gray-50 dark:bg-gray-700/60 text-xs text-gray-600 dark:text-gray-300 uppercase">
                <tr>
                    ...
                    <th class="text-left px-3 py-2.5">騎手</th>
            ...
            <tbody>
                ...
                <tr class="border-b dark:border-gray-700 hover:bg-turf-50/40 dark:hover:bg-gray-700/40 transition-colors">
                    ...
                    <td class="px-3 py-2">
                        <a href="{{ route('jockeys.show', $j) }}" class="text-turf-700 dark:text-turf-400 hover:underline font-medium">{{ $j->name }}</a>
                    </td>
                    <td class="px-3 py-2 text-gray-500 dark:text-gray-400 text-xs">{{ $j->name_kana }}</td>
                    <td class="px-3 py-2 dark:text-gray-300">
                        @if ($j->belonging)
                            <span class="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded">{{ $j->belonging }}</span>
                        @endif
                    </td>
                    <td class="px-3 py-2 text-center font-bold text-gold-600 dark:text-gold-400">{{ $j->wins ?? 0 }}</td>
                </tr>
                @empty
                <tr><td colspan="5">
                    <x-empty-state icon="user" title="該当する騎手がいません" message="検索条件を変更してください" />
                </td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div>{{ $jockeys->withQueryString()->links() }}</div>
</div>
@endsection
